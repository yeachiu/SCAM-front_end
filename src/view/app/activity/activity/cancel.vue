<template>
    <div>
        ...
        <Modal v-model="removeModal" width="360">
            ...
            <div slot="footer">
                <Button type="error" size="large" long :loading="setting.loading" @click="removeActivity">确认删除</Button>
            </div>
        </Modal>
    </div>
</template>
    ...
    export default {
              ...
              render: (h, params) => {                
                return h('div', [
                  h('Button', {
                    ...
                    on:{
                      click:()=>{
                        this.changeStatus(params.row,ACT_STATUS_DRAFT);
                      }
                    }
                  }, '恢复'),
                  h('Button', {
      ...
      methods:{
        ...
        async changeStatus(obj,currentStatus){
          this.setting.loading = true;
          let req_msg = '活动'+ obj.title +'已恢复，请去活动管理页面查看';
          let data = {
            aid : obj.id,
        ...
        async removeActivity(){
          ...
          this.setting.loading = true;
          try {
            let res = await post('/app/activity/remove/{id}',null,{
